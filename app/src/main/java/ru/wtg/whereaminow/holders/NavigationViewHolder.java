...
public class NavigationViewHolder extends AbstractViewHolder<NavigationViewHolder.NavigationView> implements Serializable {
    ...
    private void updateAll() {
        State.getInstance().getUsers().forAllUsersExceptMe(new MyUsers.Callback() {
            ...
            public void call(Integer number, MyUser myUser) {
                NavigationView view = (NavigationView) myUser.getEntity(TYPE);
                if(view != null && view.showNavigation) view.update();
            }
        });
    }
    ...
    class NavigationView extends AbstractView {
        ...
        public void onChangeLocation(final Location location) {
            if(previousLocation != null) {
                ...
                double distance = SphericalUtil.computeDistanceBetween(current, previous);

                if(distance > REBUILD_TRACK_IF_LOCATION_CHANGED_IN_METERS) {
            ...
            if(myUser == State.getInstance().getMe()) {
                State.getInstance().getUsers().forAllUsersExceptMe(new MyUsers.Callback() {
                    ...
                    public void call(Integer number, MyUser myUser) {
                        NavigationView view = (NavigationView) myUser.getEntity(TYPE);
                        if(view != null && view.showNavigation && view.track != null) {
                            view.update();
                        }
                    }
                });
            ...
            switch(event) {
                case CREATE_CONTEXT_MENU:
                    ...
                    if(!showNavigation && myUser != State.getInstance().getMe()){
                        contextMenu.add(R.string.show_navigation).setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
                            @Override
                            public boolean onMenuItemClick(MenuItem menuItem) {
                                myUser.fire(SHOW_NAVIGATION);
                                return false;
                    ...
                    if(showNavigation) {
                        contextMenu.add(R.string.hide_navigation).setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
                            @Override
                            public boolean onMenuItemClick(MenuItem menuItem) {
                                myUser.fire(HIDE_NAVIGATION);
                                return false;
                ...
                case SHOW_NAVIGATION:
                    ...
                    int color = (myUser.getProperties().getColor() & 0x00FFFFFF) | 0xAA000000;

                    track = map.addPolyline(new PolylineOptions().width((int) (6 * density)).color(color).geodesic(true).zIndex(100f));
                    ...
                    MarkerOptions markerOptions = new MarkerOptions()
                            ...
                            .position(SphericalUtil.interpolate(latLng(State.getInstance().getMe().getLocation()), latLng(myUser.getLocation()), .5))
...
//                    break;
        ...
        private LatLng latLng(Location location){
                        ...
                        new Handler(Looper.getMainLooper()).post(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if(marker != null) {
                                    ...
                                    LatLng mePosition = latLng(State.getInstance().getMe().getLocation());
                                    LatLng userPosition = latLng(myUser.getLocation());
